...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      ...
      WHERE (CAST(release_date AS integer) >= 2009)
  ...
  it(
    ...
    async done => {
      const query = `SELECT user_id, COUNT(user_id) AS count FROM movie_ratings
      GROUP BY user_id
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(*) AS count, strftime('%m', time_created) AS month FROM movie_ratings
      GROUP BY month
      ORDER BY count DESC`;
      const result = await db.selectMultipleRows(query);
